function OutputImg = AffineTransform2D3D(InputImg, TransPar)
%{ 
...
if size(InputImg, 3)==1
    ...
    %% 三线性插值
    %计算变换后坐标的领域立方体坐标（8个）
    AGPoints0 = floor(TransGPoints);
    D = TransGPoints - AGPoints0;
    DX = D(1,:)';
    DY = D(2,:)';
    DZ = D(3,:)';clear D;
    AGPoints1 =  GetPixelValue(AGPoints0,InputImg);
    AGPoints2 = AGPoints0 + [0;1;0];%010
    AGPoints2 =  GetPixelValue(AGPoints2,InputImg);
    AGPointsA = Interp_1(AGPoints1,AGPoints2,DY); clear AGPoints1 AGPoints2;
    
    AGPoints3 = AGPoints0 + [1;1;0];%110
    AGPoints3 =  GetPixelValue(AGPoints3,InputImg);
    AGPoints4 = AGPoints0 + [1;0;0];%100
    AGPoints4 =  GetPixelValue(AGPoints4,InputImg);
    AGPointsC = Interp_1(AGPoints4,AGPoints3,DY);clear AGPoints3 AGPoints4;
    
    AGPoints5 = AGPoints0 + [0;0;1];%001
    AGPoints5 =  GetPixelValue(AGPoints5,InputImg);
    AGPoints6 = AGPoints0 + [0;1;1];%011
    AGPoints6 =  GetPixelValue(AGPoints6,InputImg);
    AGPointsB = Interp_1(AGPoints5,AGPoints6,DY);clear AGPoints5 AGPoints6;
    
    AGPoints7 = AGPoints0 + [1;1;1];%111
    AGPoints7 =  GetPixelValue(AGPoints7,InputImg);
    AGPoints8 = AGPoints0 + [1;0;1];%101
    AGPoints8 =  GetPixelValue(AGPoints8,InputImg);
    AGPointsD = Interp_1(AGPoints8,AGPoints7,DY);clear AGPoints7 AGPoints8;

    AGPointsE = Interp_1(AGPointsA,AGPointsC,DX);clear AGPointsA AGPointsC;
    AGPointsF = Interp_1(AGPointsB,AGPointsD,DX);clear AGPointsB AGPointsD;
    AffineImg = Interp_1(AGPointsE,AGPointsF,DZ);clear AGPointsE AGPointsF;
